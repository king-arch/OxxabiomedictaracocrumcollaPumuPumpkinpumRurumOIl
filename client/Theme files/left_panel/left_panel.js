...
var get_userinfo_admin;
var get_userinfo_doctor;
var get_userinfo_patient;

 Template.left_panel.onDestroyed(function(){
  admin_detailed.stop();
  get_userinfo_doctor.stop();
  get_userinfo_patient.stop();
 ...
 Template.left_panel.helpers({
    ...
    display_logged_in_user_info(){
        
        var active_user_type = Session.get("active_user_type");
        var active_user = Session.get("active_user");

        if(active_user_type == 'admin'){
            get_userinfo_admin = Meteor.subscribe('get_user_info_admin',active_user);
            var result = admin_details.find({admin_id: active_user}).fetch();
        }
        else if(active_user_type == 'doctor'){
            get_userinfo_doctor = Meteor.subscribe('get_user_info_doctor',active_user);
            var result = doctor_details.find({doctor_id: active_user}).fetch();
        }
        else if(active_user_type == 'patient'){
            get_userinfo_patient = Meteor.subscribe('get_user_info_patient',active_user);
            var result = patient_details.find({patient_id: active_user}).fetch();
        }
        return result;
    },
});
...
function x_navigation(){
    ...
    $('#report_listing').click(function(){
        
                        var active_user_type = Session.get("active_user_type");
        if(active_user_type == 'doctor'){
                Router.go('/report_listing_doctor');
                }
                else if(active_user_type == 'patient'){
    ...
    $("#create_record").click(function(event){
        ...
        Router.go("/create_a_record");
    });


    $("#dashboard").click(function(event){
        event.preventDefault();
        Router.go("/dashboard_admin");
    });

    $("#logout").click(function(event){
        event.preventDefault();

        Session.setPersistent("active_user","");
        Router.go("/");
    });


    $("#load_passion").click(function(event){
        event.preventDefault();
        $('#loader_gif').removeClass("div_hide_class");
        $('#load_passion').addClass("div_hide_class");
        setTimeout(function(){
        $('#loader_gif').addClass("div_hide_class");
        $('#load_passion').removeClass("div_hide_class");

        var active_user = Session.get("active_user");
        if(Session.get("active_user_type") == 'admin'){
        var url = '/edit_profile_admin/';
        Router.go(url);
          }
        else if(Session.get("active_user_type") == 'doctor'){
        var url = '/edit_profile_doctor/'+Base64.encode(active_user);
        Router.go(url);
          }
        else if(Session.get("active_user_type") == 'patient'){
        var url = '/edit_profile_patient/'+Base64.encode(active_user);
        Router.go(url);
          }
        },1000);
    });
