...
var patient_detail_with_limit_and_search;
 ...
 Template.show_report_listing.onRendered(function(){
...
if(if_search_exists == null || if_search_exists == '' || if_search_exists == undefined){
 ...
 Template.show_report_listing.helpers({
     ...
     // console.log('check_query: '+check_query+' & skip: '+skip);
      ...
      var modality_filter = Session.get('modality_filter');
          if(modality_filter != '' || modality_filter != null ){
           ...
           Meteor.subscribe('get_report_listing');
           var reported = report.find({}).fetch({});
           ...
           for(var i=0; i < reported.length; i++){
             ...
             Meteor.subscribe('get_user_info_patient',reported[i].selected_patient);
             var new_patient = patient_details.find({ patient_name: reported[i].selected_patient } ).fetch();

             Meteor.subscribe('get_user_info_doctor',reported[i].selected_patient);
             var new_doctor = doctor_details.find({ patient_id: reported[i].selected_doctor }).fetch();

             patient_list.push(new_patient);
