...
var patient_detail_with_limit_and_search;
 ...
 Template.show_report_listing_doctor.onRendered(function(){
...
if(if_search_exists == null || if_search_exists == '' || if_search_exists == undefined){
 ...
 Template.show_report_listing_doctor.helpers({
    ...
    if(pagination == undefined || pagination == null){
     ...
     // console.log('check_query: '+check_query+' & skip: '+skip);
      ...
      var modality_filter = Session.get('modality_filter');
        ...
        var active_user = Session.get("active_user");
               var result = report.find({selected_doctor: active_user,
                                      selected_patient: 
                                                          {$in:
       ...
       swal(modality_filter);
        ...
        var active_user = Session.get("active_user");
            ...
            get_report_listing = Meteor.subscribe('get_report_listing');
           var result = report.find({selected_doctor: active_user, selected_modality: modality_filter } ).fetch();

          }else{
 swal('case 4');
                      var active_user = Session.get("active_user");
              get_report_listing = Meteor.subscribe('get_report_listing');
              var result = report.find({selected_doctor: active_user}).fetch();
              console.log('case 4');
              console.log(result);
            }
    }
